...
export function ParentSetupPage({ onBack }: ParentSetupPageProps) {
  ...
  const performSearch = async (type: 'name' | 'phone' | 'alternate') => {
    ...
    try {
      ...
      const searchTypeMap: Record<'name' | 'phone' | 'alternate', 'parentname' | 'phone' | 'alternatename'> = {
        name: 'parentname',
        phone: 'phone',
        alternate: 'alternatename',
      };

      const response = await backend.parent.searchParentCombined({
        search_text: trimmedTerm,
        search_type: searchTypeMap[type],
      });
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Enter a search term and choose how to search: by parent name (min 4 chars), phone number (10 digits), or alternate contact name (min 4 chars)
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            ...
            <Input
              id="searchTerm"
              type="text"
              placeholder="Enter parent name, phone number, or alternate contact name..."
              value={searchTerm}
